...
plt.figure()
for msid in forlist:
    target_img = np.mean(dictionary[msid]['imread'], axis=2)
    hist, bin_edges = np.histogram(target_img, bins=255, range=(0.0, 255.0))
    plt.plot(hist/np.mean(hist))
    mssave.append(hist/np.mean(hist))
    
print(np.median(np.abs(mssave[0] - mssave[1])))
print(np.median(np.abs(mssave[1] - mssave[2])))    
print(np.median(np.abs(mssave[0] - mssave[2])))    
    

mssave = []
plt.figure()
for msid in forlist:
    target_img = np.mean(dictionary2[msid]['imread'], axis=2)
    hist, bin_edges = np.histogram(target_img, bins=255, range=(0.0, 255.0))
    plt.plot(hist/np.mean(hist))
    mssave.append(hist/np.mean(hist))
...
mainpath = 'A:\\data_tmp\\20230209_en_revision_original\\'
...
for n_num in tqdm(range(len(keylist))):
    n = keylist[n_num]
    if n in forlist:
        psave = mainpath + 'data_52_ms_XYZ_2_' + str(n) + '.pickle'
        
        if not(os.path.isfile(psave)) or False:
            marker_x = dictionary2[n]['marker_x']
            marker_y = dictionary2[n]['marker_y']
            im = dictionary2[n]['imread']
            # im = np.mean(im, axis=2)
            msdict = msXYZgen(im=im, marker_x=marker_x, marker_y=marker_y)
            with open(psave, 'wb') as file:
                pickle.dump(msdict, file)
...
def mscal_entropy(tmp, msmax=None):
    import scipy
    hist, bin_edges = np.histogram(tmp, bins=255, range=(0, msmax))
...
for n in forlist:
    ...
    cell = np.where(np.array(Y)[:,0]==0)[0]
    noncell = np.where(np.array(Y)[:,0]==1)[0]
        
    X2 = np.array(X)
    
    msstd_save = [[],[]]
    for i in noncell:
        msstd_save[0].append(np.std(np.mean(X2[i], axis=2)))
    for i in cell:
        msstd_save[1].append(np.std(np.mean(X2[i], axis=2)))
    print(n, np.mean(msstd_save[0]), np.mean(msstd_save[1]))
...
for n in forlist:
    ...
    #     ms_ent_save.append(mscal_entropy(np.mean(X2[i], axis=2), msmax=msmax))
    # print(n, np.mean(ms_ent_save))
