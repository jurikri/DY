...
def msXYZgen(im=None, marker_x=None, marker_y=None):
    ...
    negative_index = list(set(total_index) - set(positive_index))
    ...
    rix = random.sample(negative_n, int(pnum)*3)
    for j in rix:
...
mainpath = 'C:\\Temp\\20221011_develop2_random_sample_control\\'
...
# if False:
#     nlist2 = np.array(pd.read_csv('C:\\Temp\\20220919_dataset123_develop2\\nlist.csv')  )
    
# for i in range(len(nlist)):
#     print(nlist[i], nlist2[i])
...
for i in range(tn):
    mean_recal_loss[i] = np.mean(BCP_save[cv][0][:i+1])
    mean_recal_f1[i] = np.mean(BCP_save[cv][1][:i+1])

plt.plot(mean_recal_loss)
plt.plot(BCP_save[cv][2])

plt.plot(mean_recal_f1)
plt.plot(BCP_save[cv][3])
...
for cv in range(0, 5): # len(cvlist)):
    ...
    for n_num in telist:
        ...
        print('test_image_no', test_image_no)
...
for n_num in range(len(nlist_for)):
    ...
    if not(os.path.isfile(psave2)) and os.path.isfile(psave):
        ...
        if False:
            marker_i = 0
            for marker_i in range(len(marker_x)):
                row = marker_y[marker_i]
                col = marker_x[marker_i]
                
                ix = np.where(np.logical_and((z_save[:,0] == row), (z_save[:,1] == col)))[0][0]
                yhat_save[ix]
