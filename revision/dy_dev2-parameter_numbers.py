...
cv = 0
for cv in range(0, cvnum): # len(cvlist)):
    if len(BCP_save[cv]) == 0:
...
for cv in range(4):
    ...
    print(cv, len(BCP_save[cv][2]))
    
    mssave8[0].append(mean_recal_loss[:364])
    mssave8[1].append(BCP_save[cv][2][:364])
    mssave8[2].append(mean_recal_f1[:364])
    mssave8[3].append(BCP_save[cv][3][:364])

plt.figure()
plt.plot(np.mean(np.array(mssave8[0]),axis=0))
plt.plot(np.mean(np.array(mssave8[1]),axis=0))

plt.plot(np.mean(np.array(mssave8[2]),axis=0))
plt.plot(np.mean(np.array(mssave8[3]),axis=0))
plt.ylim([0,1])
#%% model training
