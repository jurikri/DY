...
def msXYZgen(im=None, marker_x=None, marker_y=None):
    ...
    if False:
        ...
        plt.figure()
        plt.imshow(allo[rowmin:rowmax, colmin:colmax], cmap='binary')
        # plt.title(n)
...
mainpath = 'D:\\dy\\dy_THcelldetection\\20220919_dataset123_develop2_sh' + str(SIZE_HF) +'\\'
...
if False: # tmp 임시 model 생성용
    ...
    model = model_setup(xs=(SIZE_HF*2, SIZE_HF*2, 3), ys=2, lnn=7)
    print(model.summary())
...
#%%
csv_savepath = 'C:\\Temp\\20220919_dataset123_develop2\\merge_save.csv'
pd.DataFrame(merge_save).to_csv(csv_savepath)
